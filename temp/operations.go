...
func initalizeOperation(req SyncDataRequest, tr TypeRegistry) (*Operation, error) {
	source := map[string]SourceGroup{}
	for _, typeId := range req.Types {
		registeredType, ok := tr.Get(typeId)
		if !ok {
		}

		switch typ := registeredType.(type) {
		case StaticType:
			continue
		case UnionType:
			for _, sourceType := range typ.Types() {
